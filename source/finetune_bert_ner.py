...
dataset = load_dataset('csv', data_files={'train': ['./raw_dataset/train.txt'],
                                          "validation":['./raw_dataset/valid.txt'],
                                          "test":['./raw_dataset/test.txt']})
...
def tokenize_and_align_labels(examples):
    ...
    for token in examples["tokens"]:
        if token!=None:
            ...
            examples_tokens.append(cur_row)
            cur_row=[]
    if len(cur_row)!=0:
        examples_tokens.append(cur_row)
    ...
    examples_labels=[]
    cur_row=[]
    for token in examples["ner_tags"]:
        if token!=None:
            token = label2int[token]
            cur_row.append(token)
        else:
            examples_labels.append(cur_row)
            cur_row=[]
    if len(cur_row)!=0:
        examples_labels.append(cur_row)
    import pdb
